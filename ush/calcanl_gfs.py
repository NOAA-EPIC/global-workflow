...
def calcanl_gfs(DoIAU, l4DEnsVar, Write4Danl, ComOut, APrefix,
    ...
    if launcher == 'mpirun':
        ...
        [hosts.append(x) for x in hosts_tmp if x not in hosts]
        nhosts = len(hosts)
        ExecCMDMPI_host = 'srun  --mpi=pmi2 -n ' + str(nFH) + ' --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
        # need to account for when fewer than LEVS tasks are available
        tasks = int(os.getenv('SLURM_NPROCS', 1))
        if levs > tasks:
            ExecCMDMPILevs_host = 'srun --mpi=pmi2 -n ' + str(tasks) + ' --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
            ExecCMDMPILevs_nohost = 'srun --mpi=pmi2 -n ' + str(tasks) + ' --verbose --export=ALL'
        else:
            ExecCMDMPILevs_host = 'srun --mpi=pmi2 -n ' + str(levs) + ' --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
            ExecCMDMPILevs_nohost = 'srun --mpi=pmi2 -n ' + str(levs) + ' --verbose --export=ALL'
        ExecCMDMPI1_host = 'srun --mpi=pmi2 -n 1 --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
        ExecCMDMPI13_host = 'srun --mpi=pmi2 -n 13 --verbose --export=ALL -c 1 --distribution=arbitrary --cpu-bind=cores'
    elif launcher == 'aprun':
        hostfile = os.getenv('LSB_DJOB_HOSTFILE', '')
        with open(hostfile) as f:
            hosts_tmp = f.readlines()
